...
def run_csv2prq_Ticket_Price_Category_job():
    glue_client.start_job_run(JobName='csv2prq_Ticket_Price_Category')
    response = glue_client.get_job_runs(JobName = 'csv2prq_Ticket_Price_Category')
    job_run_id = response["JobRuns"][0]["Id"]
    while True:
        status = glue_client.get_job_run(JobName='csv2prq_Ticket_Price_Category', RunId=job_run_id)
...
def execute_snowflake_sql(**kwargs):
    ...
    file_path, file_name = get_most_recent_s3_object(
        ...
        prefix="Master_Enrich/Ticket_Price_Category/"
    )
    sql_statement = (f"CALL IMAX01_BRONZE_RAW_DEV.IMAX_MASTER.PROC_LOAD_DATA_FROM_S3_TO_BRONZE"
                     ...
                     f"'SRC_Enrich_TICKET_PRICE_CATEGORY');")
...
def execute_sf_audit(**kwargs):
    ...
    sql_statement = "CALL SF_AUDIT_DEV.ETL_AUDIT.MAIN_SP(4)"
    cursor.execute(sql_statement)
...
Csv2parquet = PythonOperator(
    task_id="Csv2parquet",
    python_callable=run_csv2prq_Ticket_Price_Category_job,
    default_args=default_args,
    dag=dag,
)
