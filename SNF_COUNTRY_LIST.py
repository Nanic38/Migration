...
def run_csv2prq_country_job():
    glue_client.start_job_run(JobName='csv2prq_country')
    response = glue_client.get_job_runs(JobName = 'csv2prq_country')
    job_run_id = response["JobRuns"][0]["Id"]
    while True:
        status = glue_client.get_job_run(JobName='csv2prq_country', RunId=job_run_id)
...
def execute_snowflake_sql(**kwargs):
    ...
    file_path, file_name = get_most_recent_s3_object(
        ...
        prefix="Master_Enrich/Country_List/"
    )
...
def execute_sf_audit(**kwargs):
    ...
    sql_statement = "CALL SF_AUDIT_DEV.ETL_AUDIT.MAIN_SP(1)"
    cursor.execute(sql_statement)
...
Csv2parquet = PythonOperator(
    task_id="Csv2parquet",
    python_callable=run_csv2prq_country_job,
    default_args=default_args,
    dag=dag,
)
