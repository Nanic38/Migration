...
# Get job parameters
args = getResolvedOptions(sys.argv, ['JOB_NAME', 'S3_SOURCE_PATH', 'S3_OUTPUT_PATH', 'S3_FILE_NAME'])

# Extract parameters
source_path = args['S3_SOURCE_PATH']
output_path = args['S3_OUTPUT_PATH']
source_filename = source_path + args['S3_FILE_NAME']
file_name = args['S3_FILE_NAME']

# Read CSV data from source path
try:
    source_data = spark.read.csv(source_filename, header=True, inferSchema=True)
...
source_data_with_filename.coalesce(1).write.parquet(output_path + "/" + current_date_str, mode='append')

# rename created file
hadoopPath = SparkContext._jvm.org.apache.hadoop.fs.Path(output_path)
...
hadoopFs.rename(SparkContext._jvm.org.apache.hadoop.fs.Path({output_path}), SparkContext._jvm.org.apache.hadoop.fs.Path(output_path + ".parquet"))
#fs.rename(
#    output_path,
#    Path(file_path + "{file_name}.parquet"))
