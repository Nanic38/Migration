...
def run_csv2prq_currency_job():
    glue_client.start_job_run(JobName='csv2prq_currency')
    response = glue_client.get_job_runs(JobName = 'csv2prq_currency')
    job_run_id = response["JobRuns"][0]["Id"]
    while True:
        status = glue_client.get_job_run(JobName='csv2prq_currency', RunId=job_run_id)
...
def execute_sf_audit(**kwargs):
    ...
    sql_statement = "CALL SF_AUDIT_DEV.ETL_AUDIT.MAIN_SP(2)"
    cursor.execute(sql_statement)
    ...
    try:
        ...
        print('ERROR in Snowflake procedure {0}'.format(err))
...
Csv2parquet = PythonOperator(
    task_id="Csv2parquet",
    python_callable=run_csv2prq_currency_job,
    default_args=default_args,
    dag=dag,
)
